...
export function MovieCardL({
    ...
    kinopoiskId,
    onClickCard
}: MovieCardLProps): React.ReactElement {
    ...
    return (
        <div onClick={onClickCard} data-id={kinopoiskId} className='relative w-full rounded-2xl flex flex-col'>
            ...
            <div className="pl-3">
                <h3 className="text-base font-semibold text-white">{nameRu}</h3>
                <span className="text-xs text-gray-400">{year}, {genres[0]?.genre}</span>
            </div>
        </div>
    )
}